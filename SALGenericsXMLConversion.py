...
def getExcelFiles(messageType, xmlFileLocation):
    excelFileLocation_Message = currdir + "/SALGenerics/SALGenerics_" + messageType + ".xlsx"
    excelFileLocation_Parameter = currdir + "/SALGenerics/SALGenerics_" + messageType + "_Properties.xlsx"
    excelFileLocation_Enumeration = currdir + "/SALGenerics/SALGenerics_" + messageType + "_Enumerations.xlsx"
    excelFileLocation_Literal = currdir + "/SALGenerics/SALGenerics_" + messageType + "_Enumerations_Options.xlsx"
...
def updateExcelFiles(xmlFileLocation, excelFileLocation_Message, excelFileLocation_Parameter, excelFileLocation_Enumeration, excelFileLocation_Literal, wb, wb_p, wb_e, wb_l, ws, ws_p, ws_e, ws_l, messageType):
   ...
   #Clearing the SyncAction Column in Signals
    ...
    for member in treeRoot.iter(messageRoot):
...
currdir = os.getcwd()
xmlFileLocation = currdir + "/XML/sal_interfaces/SALGenerics.xml"
...
logger = logging.getLogger('Rubin Observatory SALGeneric XML Message Converter')
...
if os.path.isfile(xmlFileLocation):
    #Update commands Excel files
    messageType = "Commands"
    getExcelFiles(messageType, xmlFileLocation)
    #Update events Excel files
    messageType = "Events"
    getExcelFiles(messageType, xmlFileLocation)
    #Update telemetry Excel files
    messageType = "Telemetry"
    getExcelFiles(messageType, xmlFileLocation)
else:
    logger.warning("Unable to find SALGenerics.xml file in the XML folder.")
#getXML(root, currdir)
